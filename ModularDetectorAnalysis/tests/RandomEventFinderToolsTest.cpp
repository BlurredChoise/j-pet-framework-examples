...
Coincidences getTimesOfCoincidences(const std::vector<JPetEvent> &events) {
  ...
  for (auto &event : events) {
    ...
    times.push_back(
        {event.getHits()[0]->getTime(), event.getHits()[1]->getTime()});
  }
  return times;
}
...
BOOST_AUTO_TEST_CASE(getCoincidencesFromWindows_test_00) {
  ...
  auto results =
      getCoincidencesFromWindows(window1, window2, usedHits, tofWindow);
  auto used = results.first;
  BOOST_REQUIRE_EQUAL(used.size(), 1);
  BOOST_REQUIRE_EQUAL(used[0], 0);
...
BOOST_AUTO_TEST_CASE(getCoincidencesFromWindows_test_02b) {
  ...
  auto results =
      getCoincidencesFromWindows(window1, window2, usedHits, tofWindow);
  auto used = results.first;
  BOOST_REQUIRE_EQUAL(used.size(), 2);
  BOOST_REQUIRE_EQUAL(used[0], 0);
  BOOST_REQUIRE_EQUAL(used[1], 1);
  auto events = results.second;
  BOOST_REQUIRE_EQUAL(events.size(), 1);
  auto times = getTimesOfCoincidences(events);
  BOOST_REQUIRE_CLOSE(times[0].first, 10, epsilon);
...
BOOST_AUTO_TEST_CASE(getCoincidencesFromWindows_test) {
  ...
  auto used = results.first;
  BOOST_REQUIRE_EQUAL(used.size(), 4);
  BOOST_REQUIRE_EQUAL(used[0], 1);
  BOOST_REQUIRE_EQUAL(used[1], 1);
  BOOST_REQUIRE_EQUAL(used[2], 0); // 26 not used
  BOOST_REQUIRE_EQUAL(used[3], 1);
  auto events = results.second;
